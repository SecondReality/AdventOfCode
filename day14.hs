...
distanceTravelled time (speed, sprintDuration, restDuration) = speed * (sprintDuration * div time period + min remainder sprintDuration)
...
parseReindeer str = (read speed, read sprintDuration, read restDuration)
  where (_,_,_, [_, speed, sprintDuration, restDuration]) = (str =~ "(\\w+) can fly (\\d+) km\\/s for (\\d+) seconds, but then must rest for (\\d+) seconds.") :: (String,String,String, [String])
